...
def _content_top_spacer() -> Spacer:
    if is_letterhead_available():
        return Spacer(1, HEADER_DRAW_HEIGHT + 0.2 * cm)
    return Spacer(1, 0.6 * cm)


def generate_appointment_pdf(employee: EmployeeRecord, output_path: str | Path) -> Path:
    ...
    doc = SimpleDocTemplate(
        ...
        bottomMargin=MARGIN_BOTTOM + FOOTER_DRAW_HEIGHT,
        title=f"Appointment Letter – {employee.employee_name}",
    )

    def on_page(canvas, document):
        _draw_page_decorations(canvas, document)
